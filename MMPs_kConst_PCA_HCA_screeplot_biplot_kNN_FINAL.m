...
plot(latent,'-.o') % Creates scree plot from eigenvalues contained in the latent output 
...
% Note: a biplot allows us to visualize both the orthonormal principal component coefficients for each variable (i.e. timepoint) and the principal component scores (first 2) for each observation in a single plot.
hold on
...
%% Reconstruct data and train model using the first 3 components

score_recon= score*coeff' +  repmat(mu,15,1); %reconstruct data using PCs
scoreTrain95 = score_recon(:,1:3); 

rng(1); %For reproducibility
cv = cvpartition(YTrain, 'KFold',3,'Stratify',true)
n_neighbors=2;
mdl = fitcknn(scoreTrain95,YTrain,'NumNeighbors',n_neighbors,'Distance','euclidean','CVPartition',cv); 
...
cm=confusionchart(YTrain, Predict_Labels,'RowSummary','row-normalized','ColumnSummary','column-normalized'); 
